...
function Table() {
  const { tableProperties, paginatedCustomers, sort, sortColumn, sortDesc, page, nextPage, previousPage, pageCount } = useTable()
      ...
      <table>
        <thead>
          ...
          {tableProperties.map((property) => (
            <th key={property} onClick={() => sort(property)}>{property}{property === sortColumn ? (sortDesc ? ' ▼' : ' ▲') : ''}</th>
          ))}
          </tr>
        </thead>
        <tbody>
